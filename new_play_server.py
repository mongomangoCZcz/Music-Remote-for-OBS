...
<body>
  ...
  function updateProgress() {
    ...
    if (elapsed >= totalSeconds) {
      ...
      document.getElementById("currentTime").textContent = formatTime(totalSeconds);
      clearInterval(intervalId); // Oprava: ukončí aktualizaci
      return;
    }
...
def index():
    ...
    playing = ffplay_process is not None and ffplay_process.poll() is None

    if request.method == 'POST':
        query = request.form.get('query')
        if not query:
            return render_template_string(HTML, title=None, thumbnail=None, playing=False)
        try:
            ...
            return render_template_string(HTML + f"<p style='color:red;'>Chyba: {e}</p>", title=None, thumbnail=None, playing=False)
